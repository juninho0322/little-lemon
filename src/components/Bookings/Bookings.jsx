...
export const Bookings = ({ open, onClose, onSubmit }) => {
  ...
  useEffect(() => {
    if (!date) return;
    const d = new Date(date + "T00:00:00"); // avoid timezone weirdness
    const times = fetchAPI(d);
...
const handleSubmit = (e) => {
  ...
  const ok = submitAPI(payload);
  if (!ok) return; // or show an error UI
                  ...
                  <Field>
                    ...
                    <Input
                      ...
                      onChange={(e) => {
                        ...
                        setGuests(next === "" ? "" : clamp(Number(next), 1, 25));
                      }}
